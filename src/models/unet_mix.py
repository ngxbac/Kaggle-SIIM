...
class UnetDecoderMix(Model):
    ...
    def forward(self, x):
        ...
        bs = encoder_head.size(0)

        cls_encoder_head = F.adaptive_avg_pool2d(encoder_head, output_size=1).view(bs, -1)  # 512
        cls_encoder_head = self.dropout(cls_encoder_head)
        x_cls = self.fuse_image(cls_encoder_head)
...
class UnetMix(EncoderDecoder):
    def __init__(self,
        ...
        self.name = 'umix-{}'.format(encoder_name)
